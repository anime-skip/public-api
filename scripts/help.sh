...
makeRule "make, make build   " "Build the source code to $(underline bin/anime-skip-api) (default, can be ran with just $(codeBlock make))"
makeRule "make run           " "Run the server"
makeRule "make watch         " "Run the server and restart it when a file changes"

makeRule "make services      " "Start database and other services the backend relies on"
makeRule "make reset-services" "Delete database data and reset other serivces cached data"

makeRule "make gen           " "Run gqlgen to generate GraphQL models and resolvers"
makeRule "make clean         " "Clean go's cache and redownload go dependencies"
